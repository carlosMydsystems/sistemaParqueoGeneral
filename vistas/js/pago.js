...
window.onload = function () {
	...
	let codigoBarras = $("#idEstacionamiento").val()
		...
		if (rucCliente.length != 11 && tipoDocumento == "FACTURA") {
			...
			$.ajax({
				...
				success: function (respuesta) {
					...
					let idResumenBoleta = $("#idResumenBoleta").val(); 

					alert("el resumen a registrar es " + idResumenBoleta)

					if (tipoDocumento == "BOLETA") {

						if (dniCliente.length == 0) { dniCliente = "12345678" }
						if (direccionFiscalCliente.length <= 0) { direccionFiscalCliente = "-" }
					}
					...
					datos.append("idResumenBoleta", idResumenBoleta);
					...
					$.ajax({
						...
						success: function (respuesta) {
							...
							window.location = "index.php?ruta=comprobante&idEstacionamiento=" + idEstacionamiento;
...
function obtenerCorrelativo() {
	let datos = new FormData();
	//datos.append("tipoDocumento", tipoDocumento);
	datos.append("accion", "mostrarCorrelativoResumen");
	...
	$.ajax({
		...
		success: function (respuesta) {
			if (respuesta == false) {
				...
				alert(numeroCorrelativoResumen) 
			} else {
				numeroCorrelativoResumen = respuesta
				alert(numeroCorrelativoResumen['correlativo']) 
			}
		}
	})
